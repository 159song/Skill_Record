...
def ulaw_to_pcm(ulaw_bytes: bytes, in_sample_rate: int, out_sample_rate: int):
    ...
    # Resample
    out_pcm_bytes = audioop.ratecv(
        in_pcm_bytes, 2, 1, in_sample_rate, out_sample_rate, None
    )[0]
...
def pcm_to_ulaw(pcm_bytes: bytes, in_sample_rate: int, out_sample_rate: int):
    # Resample
    in_pcm_bytes = audioop.ratecv(
        pcm_bytes, 2, 1, in_sample_rate, out_sample_rate, None
    )[0]
