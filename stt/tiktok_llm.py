...
class AsrWsClient:
    ...
    async def segment_data_processor(self, wav_data: bytes, segment_size: int):
        ...
        header["X-Api-Access-Key"] = "DDFAiCUS0XZ5qUzBEsmC4lvihyIKcO10"

        header["X-Api-App-Key"] = "4227051015"
        ...
        try:
            async with websockets.connect(
                ...
                # 移除 response_headers 的打印
                # print(ws.response_headers)  # 删除这行
                ...
                for _, (chunk, last) in enumerate(
                    ...
                    req_str = " ".join(
                        format(byte, "02x") for byte in audio_only_request
                    )
                    ...
                    result = parse_response(res)
                    print(
                        f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}, seq",
                        seq,
                        "res",
                        result,
                    )
...
def execute_one(audio_item, **kwargs):
    ...
    asr_http_client = AsrWsClient(audio_path=audio_path, **kwargs)
...
def test_stream():
    print("测试流式")
    result = execute_one({"id": 1, "path": "test.wav"})
    print(result)
